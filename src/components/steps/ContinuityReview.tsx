...
import { callGemini, getPlanningModel, imageUrlToBase64 } from '@/lib/google-ai';
...
export function ContinuityReview() {
  const { scenes, continuityFlags, setContinuityFlags, goToNextStep, goToPrevStep, qualityMode, transitions } = useProjectStore();
  ...
  const handleRunCheck = async () => {
    ...
    try {
      ...
      // 3. Check repair scenes have worker cues in prompts
      REPAIR_SCENES.forEach(idx => {
        const scene = scenes[idx];
        if (!scene) return;
        const hasWorkerRef = /scaffold|tool|equipment|welding|construction|machinery|cable|debris|paint|mount/i.test(scene.imagePrompt);
        if (!hasWorkerRef) {
          flags.push({
            sceneIndex: idx,
            type: 'worker-logic',
            message: `Scene ${idx + 1} (${scene.title}): Repair scene but no worker/tool evidence in prompt. Structural changes require visible construction cues.`,
            severity: 'warning',
          });
        }
      });
      ...
      ATMOSPHERE_ONLY_SCENES.forEach(idx => {
        const scene = scenes[idx];
        if (!scene) return;
        const hasRepairRef = /repair|fix|restore|construct|build|install/i.test(scene.imagePrompt);
        if (hasRepairRef) {
          flags.push({
            sceneIndex: idx,
            type: 'worker-logic',
            message: `Scene ${idx + 1} (${scene.title}): Atmosphere-only scene contains repair language. Only dust, light, ambience allowed.`,
            severity: 'warning',
          });
        }
      });
      ...
      if (scenesWithImages.length >= 2) {
        try {
          // Build image descriptions for Gemini analysis
          const sceneDescriptions = scenes.map((s, i) => 
            `Scene ${i + 1} (${s.title}): ${s.generatedImageUrl ? 'Has image' : 'No image'} — Prompt: ${s.imagePrompt.substring(0, 100)}...`
          ).join('\n');
  ...
  return (
    <div className="flex flex-col gap-4 pb-24">
      <div className="px-1">
        ...
        <p className="text-sm text-muted-foreground">Review all 9 scenes for drift, worker logic, and consistency.</p>
      </div>

      {/* Scene type legend */}
      <div className="flex gap-3 px-1 text-[10px]">
        <span className="flex items-center gap-1">
          <span className="w-2 h-2 rounded-full bg-primary" /> Repair Scene
        </span>
        <span className="flex items-center gap-1">
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-background/80 px-1 py-0.5">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[10px] font-semibold truncate">{idx + 1}. {scene.title}</span>
                </div>
              </div>
              {flags.length > 0 && (
        ...
        <WorkshopCard>
          <div className="flex items-center justify-between mb-2">
            <div>
              ...
              <span className={`text-[10px] font-semibold ${REPAIR_SCENES.includes(selectedScene) ? 'text-primary' : 'text-muted-foreground'}`}>
                {REPAIR_SCENES.includes(selectedScene) ? '🔧 Repair Scene — Workers Required' : '🌫️ Atmosphere Only'}
              </span>
            </div>
          ...
          {REPAIR_SCENES.includes(selectedScene) && scenes[selectedScene].workerCues?.length > 0 && (
            <div className="mb-2 p-2 rounded bg-primary/10">
              <p className="text-[10px] font-semibold text-primary mb-1">Required Worker Cues:</p>
