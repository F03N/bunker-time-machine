...
export function IdeaGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setIdeas(parsed);
      toast.success('Generated 10 bunker concepts across 10 environments');
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-4 pb-24">
      <div className="px-1">
        ...
        <p className="text-sm text-muted-foreground">Generate 10 unique bunker concepts across different environments. Choose one.</p>
      </div>
        ...
        <WorkshopCard generating={generating}>
          <div className="text-center py-8">
            <p className="text-muted-foreground mb-4 text-sm">
              {generating ? 'Generating 10 bunker concepts (mountain, desert, coastal, forest, snow, city, jungle, cliffside, industrial, warzone)…' : 'Ready to generate 10 unique environment concepts from master prompt.'}
            </p>
            {!generating && (
              <button
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground flex-wrap">
                      ...
                      {idea.environmentType && (
                        <span className="flex items-center gap-1"><Globe className="w-3 h-3" />{idea.environmentType}</span>
                      )}
                    </div>
