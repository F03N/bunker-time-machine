...
export function PairTransitionStudio() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const result = await callVeo({
        prompt: fullPrompt, model: getVideoModel(qualityMode),
  ...
  return (
    <div className="flex flex-col gap-4 pb-24">
      ...
      {/* Veo behavior banner — always visible */}
      <div className="mx-1 p-2 rounded-md bg-muted border border-border">
        <div className="flex items-start gap-2">
          <Info className="w-4 h-4 text-muted-foreground shrink-0 mt-0.5" />
          <div className="text-[10px] text-muted-foreground">
            <p className="font-semibold text-foreground mb-0.5">How Veo Pair Transitions Work</p>
            <p>• <strong>Image A</strong> = start frame (sent as initial image)</p>
            <p>• <strong>Image B</strong> = visual target (sent as STYLE_IMAGE reference)</p>
            <p>• <strong>Motion prompt</strong> = describes the change between them</p>
            <p className="mt-1 text-yellow-600">⚠ Veo does NOT lock the end frame to Image B. It uses Image B as a visual guide toward the target state.</p>
          </div>
        </div>
      </div>
      ...
      <WorkshopCard className="rounded-t-none border-t-0">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground font-semibold uppercase tracking-wide">End Frame <span className="text-yellow-600 normal-case">(visual guide)</span></span>
          </div>
      ...
      <StickyAction label="Continue to Audio" onClick={goToNextStep} disabled={!allApproved} secondary={{ label: 'Back', onClick: goToPrevStep }} />
    </div>
  );
}
